...
def _list_motion_groups(model3_data: dict) -> dict[str, int]:
    """Read FileReferences.Motions → {group_name: count}."""
    fr = model3_data.get("FileReferences") or {}
    motions = fr.get("Motions") or {}
    return {name: len(entries) for name, entries in motions.items()}


def _suggest_motion_mapping(motion_groups: dict[str, int]) -> dict[str, dict]:
    """Heuristic: if the model has Tap/Flick-style groups (typical of Live2D
    SDK samples), bind a few emotions to a sensible group as a fallback.
    These are best-guess — users should review and re-bind.

    Skips Idle (it's the background loop, not a reaction)."""
    if not motion_groups:
        return {}
    available = {g for g in motion_groups if g.lower() != "idle"}
    out: dict[str, dict] = {}

    def take(group_name: str, emotion: str) -> None:
        # Match case-insensitively across the actual group set.
        match = next((g for g in available if g.lower() == group_name.lower()), None)
        if match:
            out[emotion] = {"group": match, "index": 0}
...
def _write_sidecar(
    ...
    motion_suggestions = _suggest_motion_mapping(motion_groups)
    ...
    lines = [
        ...
        "",
        "# emotion → expression name. Tried first when the LLM emits an emotion tag.",
        "emotion_mapping:",
    ]
    if suggestions:
    ...
    # motion_mapping: fallback when expression isn't available for an emotion.
    lines.append("")
    lines.append("# emotion → motion fallback. Used when emotion_mapping has no")
    lines.append("# entry, or when the model has no expression data at all.")
    lines.append("motion_mapping:")
    if motion_suggestions:
        for emo, m in motion_suggestions.items():
            lines.append(f'  {emo}: {{ group: "{m["group"]}", index: {m["index"]} }}')
    else:
        lines.append("  # (no motion groups detected)")

    if motion_groups:
        lines.append("")
        lines.append(f"# Available motion groups: "
                     f"{', '.join(f'{g}({n})' for g, n in motion_groups.items())}")
...
def main() -> int:
    ...
    model3_data = _read_json(model3) or {}
    motion_groups = _list_motion_groups(model3_data)
    print(f"  motions:  {sum(motion_groups.values())} files across "
          f"{len(motion_groups)} groups: {list(motion_groups.keys())}")
    ...
    _write_sidecar(folder, model3, mouth_param, expression_entries,
                   motion_groups, args.dry_run, args.force)
    print()
    print("== done ==")
    if args.dry_run:
